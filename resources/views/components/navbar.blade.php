             ...
             <div class="hidden md:flex items-center gap-14">
                 ...
                 @if (Auth::check())
                     <button id="dropdownDefaultButton" data-dropdown-toggle="dropdown"
                         ...
                         type="button">{{ ucfirst(Auth::user()->username) }} <svg class="w-2.5 h-2.5 ms-3"
                         ...
                         <ul class="py-2 text-sm text-gray-700 dark:text-gray-200"
                             ...
                             <li>
                                 <form action="{{ route('logout-user') }}" method="POST">
                                     @csrf
                                     <button type="submit" class="block w-full text-left px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Sign out</button>
                                 </form>
                             </li>
                         </ul>
                     </div>
